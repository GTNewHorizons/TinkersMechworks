...
public class SignalBus extends Block implements ITileEntityProvider {
    ...
    private static AxisAlignedBB[] getBoxes(SignalBusLogic logic) {
        ...
        if (placed[ForgeDirection.DOWN.ordinal()]) {
            ...
            renderDir = new boolean[] { (connected[0] || placed[0] || corners[0]),
                    (connected[1] || placed[1] || corners[1]), (connected[2] || placed[2] || corners[2]),
                    (connected[3] || placed[3] || corners[3]), (connected[4] || placed[4] || corners[4]),
    ...
    private static int closestClicked(EntityPlayer player, double reachDistance, SignalBusLogic terminal,
            AxisAlignedBB[] parts) {
        int closest = -1;
    ...
    public boolean canPlaceBlockOnSide(World par1World, int par2, int par3, int par4, int par5) {
        ...
        return solid;
        // return (dir == DOWN && par1World.isSideSolid(par2, par3 + 1, par4, DOWN)) || (dir == UP &&
        // par1World.isSideSolid(par2, par3 - 1, par4, UP))
        // || (dir == NORTH && par1World.isSideSolid(par2, par3, par4 + 1, NORTH)) || (dir == SOUTH &&
        // par1World.isSideSolid(par2, par3, par4 - 1, SOUTH))
        // || (dir == WEST && par1World.isSideSolid(par2 + 1, par3, par4, WEST)) || (dir == EAST &&
