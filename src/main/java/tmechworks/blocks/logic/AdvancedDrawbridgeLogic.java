...
public class AdvancedDrawbridgeLogic extends InventoryLogic
    ...
    // This gets called too early. Adding the fake player here creates multiple copies of the TileEntity and causes
    // havoc!
    /*
     * @Override public void validate () { this.tileEntityInvalid = false; fakePlayer = new
     * FakePlayerLogic("Player.Drawbridge", this); }
     */
    ...
    private void setFakePlayerRotation() {
        if (placementDirection == 4) {
            ...
            if (this.direction == 0) fakePlayer.rotationPitch = -90;
            else fakePlayer.rotationPitch = 90;
        } else {
            if (placementDirection == 0) // Forward
            {
                fakePlayer.rotationYaw = mapDirection() * 90;

                if (placementDirection == 0) fakePlayer.rotationPitch = 90;
                else fakePlayer.rotationPitch = -90;
            } else if (placementDirection == 2) // Backward
            {
                ...
                if (placementDirection == 1) facing += 1;
                else facing -= 1;
    ...
    public ItemStack decrStackSize(int slot, int quantity) {
        ...
        if (slot == 1) worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
        return stack;
    }
    ...
    public void updateEntity() {
        if (working && !isInvalid() && !worldObj.isRemote) {
            ...
            if (ticks == 5) {
                ...
                if (active) // Placement
                {
                    if (getStackInSlot(extension) != null && getStackInSlot(extension).stackSize > 0
                            && extension < 15) {
                        extension++;
                        int xPos = xCoord;
                        int yPos = yCoord;
                        ...
                        if (xPos < -30000000 || zPos < -30000000
                                || xPos >= 30000000
                                || zPos >= 30000000
                                || yPos < 0
                                || yPos >= 256) {
                        ...
                        if (block == null || block.isAir(worldObj, xPos, yPos, zPos)
                                || block.canPlaceBlockAt(worldObj, xPos, yPos, zPos)) {
                            // tryExtend(worldObj, xPos, yPos, zPos, direction);
                            initFakePlayer();
                            setFakePlayerRotation();
                            Item blockToItem = getStackInBufferSlot(extension - 1) != null
                                    && getStackInBufferSlot(extension - 1).getItem() != null
                                            ? TMechworksRegistry.blockToItemMapping.get(
                                                    Block.getBlockFromItem(
                                                            getStackInBufferSlot(extension - 1).getItem()))
                                            : Items.stick;
                            if (blockToItem == Item.getItemFromBlock(Blocks.air)) {
                                if (getStackInSlot(extension - 1) == null
                                        || Block.getBlockFromItem(getStackInSlot(extension - 1).getItem()) == null)
                                    return;
                                ItemStack placeStack = getStackInBufferSlot(extension - 1);
                                if (placeStack != null) {
                                    Block placeBlock = Block.getBlockFromItem(placeStack.getItem());
                                    placeBlockAt(
                                            getStackInSlot(extension - 1),
                                            fakePlayer,
                                            worldObj,
                                            xPos,
                                            yPos,
                                            ...
                                            0,
                                            0,
                                            getStackInSlot(extension - 1).getItemDamage(),
                                            placeBlock);
                                }
                            } else {
                            ...
                            AxisAlignedBB axisalignedbb = axis != null
                                    ? axis.getCollisionBoundingBoxFromPool(worldObj, xPos, yPos, zPos)
                                    : null;
                            ...
                            if (axisalignedbb != null) {
                                ...
                                if (!list.isEmpty()) {
                                    ...
                                    while (iterator.hasNext()) {
                                        ...
                                        entity.moveEntity(
                                                Facing.offsetsXForSide[this.direction],
                                                Facing.offsetsYForSide[this.direction],
                                                Facing.offsetsZForSide[this.direction]);
                                    }
                ...
                // Retraction
                {
                    if ((getStackInSlot(extension) == null
                            || getStackInSlot(extension).stackSize < getStackInSlot(extension).getMaxStackSize())
                            && extension > 0) {
                        int xPos = xCoord;
                        int yPos = yCoord;
                        int zPos = zCoord;
                        ...
                        if (block != null) {
                            ...
                            if (getStackInBufferSlot(extension - 1) != null && validBlock(extension - 1, block)
                                    && validMetadata(extension - 1, block, meta)
                                ...
                                if (worldObj.setBlock(xPos, yPos, zPos, Blocks.air))
                                    if (getStackInSlot(extension - 1) == null) {
                                        setInventorySlotContents(
                                                extension - 1,
                                                getStackInBufferSlot(extension - 1).copy());
                                    } else {
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
            float hitX, float hitY, float hitZ, int metadata, Block block) {
    ...
    boolean validDrawbridge(int x, int y, int z) {
        ...
        if (te instanceof IDrawbridgeLogicBase && ((IDrawbridgeLogicBase) te).hasExtended()) return false;
    ...
    boolean validBlock(int slot, Block block) {
        ...
        if (type != Blocks.air) {
            if (type == Block.getBlockFromItem(getStackInBufferSlot(slot).getItem())) return true;
        }
        Item blockToItem = TMechworksRegistry.blockToItemMapping.get(new ItemStack(block).getItem());
        if (blockToItem != Item.getItemFromBlock(Blocks.air)) {
            if (blockToItem == getStackInBufferSlot(slot).getItem()) return true;
        }
    ...
    boolean validMetadata(int slot, Block block, int metadata) {
        /**
         * int type = TMechworksRegistry.drawbridgeState.get(block).getTypeID(); if (type == 0) { return metadata ==
         * getStackInBufferSlot(slot).getItemDamage(); } if (type == 1) { return true; } if (type == 2) { return false;
         * } if (type == 3) { return true; // TODO: rotational metadata, probably not needed // anymore } if (type == 4)
         * { return true; } if (type == 5) { return metadata == getStackInBufferSlot(slot).getItemDamage(); }
         */
        return true;
    }
    ...
    public void readBufferFromNBT(NBTTagCompound tags) {
        ...
        // bufferStacks.ensureCapacity(nbttaglist.tagCount() > getSizeInventory() ? getSizeInventory() :
        // nbttaglist.tagCount());
    ...
    public class InvCamo extends InventoryBasic {
        ...
        public void setInventorySlotContents(int slot, ItemStack content) {
            ...
            if (slot == 0 && worldObj != null) worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
        }
