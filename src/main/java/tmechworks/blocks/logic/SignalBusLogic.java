...
public class SignalBusLogic extends MultiblockBaseLogic implements ISignalBusConnectable {
        ...
        for (i = 0; i < 6; ++i) {
            ...
            for (j = 0; j < 6; ++j) {
                ...
                if (te instanceof IMultiblockMember && ((IMultiblockMember) te).isCompatible((Object) this)
                        && ((IMultiblockMember) te).willConnect(getCoordInWorld())) {
            ...
            if (worldObj.getTileEntity(x, y, z) instanceof ISignalTransceiver) {
                ...
                if (dropWire > 0) {
                    ...
                    EntityItem entityitem = new EntityItem(
                            world,
                            (double) ((float) xCoord + jumpX),
                            (double) ((float) yCoord + jumpY),
                            (double) ((float) zCoord + jumpZ),
                            tempStack);
    ...
    public boolean[] connectedSides(ForgeDirection fromFace) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            connected[i] = (te instanceof ISignalBusConnectable
                    && ((ISignalBusConnectable) te).connectableOnFace(fromFace));
        }
        ...
        switch (dir) {
            case DOWN:
                return (this.worldObj
                        .getTileEntity(this.xCoord, this.yCoord - 1, this.zCoord) instanceof SignalBusLogic);
            case NORTH:
                return (this.worldObj
                        .getTileEntity(this.xCoord, this.yCoord, this.zCoord - 1) instanceof SignalBusLogic);
            case SOUTH:
                return (this.worldObj
                        .getTileEntity(this.xCoord, this.yCoord, this.zCoord + 1) instanceof SignalBusLogic);
            case WEST:
                return (this.worldObj
                        .getTileEntity(this.xCoord - 1, this.yCoord, this.zCoord) instanceof SignalBusLogic);
            case EAST:
                return (this.worldObj
                        .getTileEntity(this.xCoord + 1, this.yCoord, this.zCoord) instanceof SignalBusLogic);
            default:
    ...
    public boolean[] getRenderCorners(ForgeDirection fromFace) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            te = worldObj.getTileEntity(neighborX, neighborY, neighborZ);
            corners[i] = (te instanceof ISignalBusConnectable
                    && ((ISignalBusConnectable) te).connectableOnCorner(fromFace.getOpposite(), dir.getOpposite()));
        }
    ...
    public boolean willConnect(CoordTuple coord) {
        ...
        for (i = 0; i < 6; ++i) {
            ...
            for (j = 0; j < 6; ++j) {
                ...
                if (xCoord + jDir.offsetX == coord.x && yCoord + jDir.offsetY == coord.y
                        && zCoord + jDir.offsetZ == coord.z) {
                ...
                if (worldObj.getBlock(xCoord + jDir.offsetX, yCoord + jDir.offsetY, zCoord + jDir.offsetZ)
                        .isOpaqueCube()) {
                    continue;
                }
                if (xCoord + iDir.offsetX + jDir.offsetX == coord.x && yCoord + iDir.offsetY + jDir.offsetY == coord.y
                        && zCoord + iDir.offsetZ + jDir.offsetZ == coord.z) {
                    te = worldObj.getTileEntity(
                            xCoord + iDir.offsetX + jDir.offsetX,
                            yCoord + iDir.offsetY + jDir.offsetY,
                            zCoord + iDir.offsetZ + jDir.offsetZ);
                    if (te instanceof ISignalBusConnectable && ((ISignalBusConnectable) te)
                            .connectableOnCorner(iDir.getOpposite(), jDir.getOpposite())) {
                        return true;
                    }
                }
            }
