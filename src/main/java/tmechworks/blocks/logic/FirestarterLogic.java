        ...
        if (active) {
            ...
            if (block == null || WorldHelper.isAirBlock(worldObj, xPos, yPos, zPos)) {
                worldObj.playSoundEffect(
                        ...
                        "fire.ignite",
                        1.0F,
                        AbilityHelper.random.nextFloat() * 0.4F + 0.8F);
                worldObj.setBlock(xPos, yPos, zPos, Blocks.fire);
            }
        } else {
            // TConstruct.logger.info("Stopping fire "+putOut);
            if (block == Blocks.fire) {
                // worldObj.playSoundEffect((double) xPos + 0.5D, (double) yPos + 0.5D, (double) zPos + 0.5D,
                // "random.fizz", 1.0F, AbilityHelper.random.nextFloat() * 0.4F + 0.8F);
                worldObj.playSoundEffect(
                        (double) xPos + 0.5D,
                        (double) yPos + 0.5D,
                        (double) zPos + 0.5D,
