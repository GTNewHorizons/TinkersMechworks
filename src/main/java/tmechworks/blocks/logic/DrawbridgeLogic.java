...
public class DrawbridgeLogic extends InventoryLogic implements IFacingLogic, IActiveLogic, IDrawbridgeLogicBase {
    ...
    // havoc!
    /*
     * @Override public void validate () { this.tileEntityInvalid = false; fakePlayer = new FakePlayerLogic(new
     * GameProfile(null, "Player.Drawbridge"), this); }
     */
                ...
                if (active) // Placement
                {
                    if (inventory[0] != null && inventory[0].stackSize > 0 && extension < maxExtension) {
                        ...
                        if (block == null || block.isAir(worldObj, xPos, yPos, zPos)
                            ...
                            initFakePlayer();
                            setFakePlayerRotation();
                            Item blockToItem = TMechworksRegistry.blockToItemMapping
                                    .get(Item.getIdFromItem(bufferStack.getItem()));
                            if (blockToItem == Item.getItemFromBlock(Blocks.air)) {
                                if (Block.getBlockFromItem(inventory[0].getItem()) == null) return;
                            ...
                            AxisAlignedBB axisalignedbb = Block.getBlockFromItem(bufferStack.getItem())
                                    .getCollisionBoundingBoxFromPool(worldObj, xPos, yPos, zPos);
                ...
                // Retraction
                {
                    if ((inventory[0] == null || inventory[0].stackSize < inventory[0].getMaxStackSize())
                        ...
                        if (block != null) {
                            ...
                            if (bufferStack != null && validBlock(block)
                                    && validMetadata(block, meta)
                                    && validDrawbridge(xPos, yPos, zPos)
                                    && !hasInventory(xPos, yPos, zPos)) {
                                worldObj.playSoundEffect(
                                        (double) xPos + 0.5D,
                                        (double) yPos + 0.5D,
                                        (double) zPos + 0.5D,
                                ...
                                if (worldObj.setBlock(xPos, yPos, zPos, Blocks.air)) if (inventory[0] == null) {
    ...
    boolean validBlock(Block block) {
        ...
        if (type != Blocks.air) {
            if (type == Block.getBlockFromItem(bufferStack.getItem())) return true;
        }
        Item blockToItem = TMechworksRegistry.blockToItemMapping.get(new ItemStack(block).getItem());
        if (blockToItem != Item.getItemFromBlock(Blocks.air)) {
            if (blockToItem == bufferStack.getItem()) return true;
        }
    ...
    boolean validMetadata(Block block, int metadata) {
         ...
         * bufferStack.getItemDamage(); } if (type == 1) { return true; } if (type == 2) { return false; } if (type ==
         * 3) { return true; //TODO: rotational metadata, probably not needed anymore } if (type == 4) { return true; }
    ...
    boolean hasInventory(int x, int y, int z) {
        TileEntity te = worldObj.getTileEntity(x, y, z);
        if (te != null && (te instanceof IInventory || te instanceof ISidedInventory)) return true;
    ...
    public boolean isItemValidForSlot(int slot, ItemStack itemstack) {
        ...
        if (slot == 1) {
            if (inventory[slot] != null || itemstack.stackSize > 1) return false;
