...
public abstract class MultiblockBaseLogic extends TileEntity implements IMultiblockMember {
    ...
    public IMultiblockMember[] getNeighboringMembers() {
        ...
        for (CoordTuple neighbor : neighbors) {
            ...
            if (te instanceof IMultiblockMember) {
                ...
                if (((IMultiblockMember) te).isCompatible((Object) this)
                ...
                if (targetMaster == null) {
                    ...
                    if (comparison < 0) {
                        ...
                        throw new IllegalStateException(
                                String.format(
                                        "Found two controllers (hashes %d,  %d) with identical reference coord %s",
                                        targetMaster.hashCode(),
                                        candidateMaster.hashCode(),
    ...
    public void onOrphaned() {
        ...
        for (CoordTuple coord : neighborCoords) {
            ...
            if (neighborTE instanceof IMultiblockMember && !((IMultiblockMember) neighborTE).isConnected()
                    && ((IMultiblockMember) neighborTE).willConnect(getCoordInWorld())) {
    ...
    public void validate() {
        ...
        if (!this.worldObj.isRemote) {
            MultiblockRegistry
                    .registerMember(this.worldObj, ChunkCoordIntPair.chunkXZ2Int(xCoord >> 4, zCoord >> 4), this);

            if (!this.worldObj.getChunkProvider().chunkExists(xCoord >> 4, zCoord >> 4)) {
                boolean master = this.cachedMultiblockData != null;
                MultiblockRegistry.onMemberLoad(
                        this.worldObj,
                        ChunkCoordIntPair.chunkXZ2Int(xCoord >> 4, zCoord >> 4),
                        this,
                        master);
            }
        }
    ...
    protected CoordTuple[] getNeighborCoords() {
        return new CoordTuple[] { new CoordTuple(this.xCoord - 1, this.yCoord, this.zCoord),
                new CoordTuple(this.xCoord, this.yCoord - 1, this.zCoord),
                new CoordTuple(this.xCoord, this.yCoord, this.zCoord - 1),
                new CoordTuple(this.xCoord, this.yCoord, this.zCoord + 1),
                new CoordTuple(this.xCoord, this.yCoord + 1, this.zCoord),
