...
public class AdvDrawbridgeGui extends GuiContainer {
    ...
    private static final ResourceLocation background = new ResourceLocation(
            "tmechworks",
            "textures/gui/drawbridgeAdvanced.png");
        ...
        if (!isGuiExpanded) {
            ...
            for (int index = 0; index < 16; index++) {
                drawTexturedModalRect(
                        index < 8 ? cornerX + 9 + 20 * index : cornerX + 9 + 20 * (index - 8),
                        cornerY + 34 + (int) Math.floor(index / 8) * 18 + (index < 8 ? 0 : 1),
                        238,
                        0,
                        18,
                        18);
                if (index != 15) {
                    drawTexturedModalRect(
                            index < 8 ? cornerX + 26 + 20 * index : cornerX + 26 + 20 * (index - 8),
                            cornerY + 34 + (int) Math.floor(index / 8) * 18 + (index < 8 ? 0 : 1) + 6,
                            253,
                            18,
                            3,
                            5);
    ...
    public void initGui() {
        ...
        if (logic.getPlacementDirection() == 0) button.enabled = false;
        this.buttonList.add(button);
        button = new DrawbridgeButton(1, cornerX + 146, cornerY + 34, 199, 23, 22, 21);
        if (logic.getPlacementDirection() == 1) button.enabled = false;
        this.buttonList.add(button);
        button = new DrawbridgeButton(2, cornerX + 132, cornerY + 48, 199, 0, 21, 22);
        if (logic.getPlacementDirection() == 2) button.enabled = false;
        this.buttonList.add(button);
        button = new DrawbridgeButton(3, cornerX + 117, cornerY + 34, 178, 23, 22, 21);
        if (logic.getPlacementDirection() == 3) button.enabled = false;
        this.buttonList.add(button);
        button = new DrawbridgeButton(4, cornerX + 135, cornerY + 40, 217, 0, 10, 10);
        if (logic.getPlacementDirection() == 4) button.enabled = false;
        this.buttonList.add(button);

        this.buttonList.add(
                new AdvDrawbridgeButton(
                        5,
                        this.width / 2 - 13,
                        this.height / 2 - 52,
                        this.width / 2 + 58,
                        ...
                        26,
                        26,
                        StatCollector.translateToLocal("container.inv")));
    }
    ...
    void updateServer(byte direction) {
         ...
         * DataOutputStream(bos); try { outputStream.writeByte(5);
         * outputStream.writeInt(logic.getWorld().provider.dimensionId); outputStream.writeInt(logic.xCoord);
         * outputStream.writeInt(logic.yCoord); outputStream.writeInt(logic.zCoord); outputStream.writeByte(direction);
         * } catch (Exception ex) { ex.printStackTrace(); } Packet250CustomPayload packet = new
         * Packet250CustomPayload(); packet.channel = "TMechworks"; packet.data = bos.toByteArray(); packet.length =
         * bos.size(); PacketDispatcher.sendPacketToServer(packet);
         */

        TMechworks.packetPipeline
                .sendToServer(new PacketDrawbridge(logic.xCoord, logic.yCoord, logic.zCoord, direction));
    }
